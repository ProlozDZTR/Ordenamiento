/*
  ...
  Compilación:
   - Windows: gcc Insertion.c -o Insertion.exe
   - Linux:   gcc Insertion.c -o Insertion
*/
...
{
	int i;
	for (i=0; i<= n-1; i++) //se recorre todo el arreglo
	{
		int j=i;
		int temp= A[i]; // guarda el valor 
		
		while(j> 0 && (temp<A[j-1])) //mueve los elementos mayores a la derecha
		{
			A[j]= A[j-1];
			j--;
		}
		A[j]= temp; //se inserta el valor en su posición correcta
	}	
}
